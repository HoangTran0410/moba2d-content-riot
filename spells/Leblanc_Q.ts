...
export const isSigilTarget = (target: unknown): target is SigilTarget =>
 ...
 */
export class Leblanc_Q_Mark extends Buff {
  name = 'Ấn Ký Ác Ý';
...
export class Leblanc_Q_Object extends HomingMissileSpellObject {
  damage = DAMAGE;
  markDurationMs = MARK_DURATION_MS;
  ...
  onTargetArrive(target: SigilTarget): void {
    ...
    if (existingMark) {
      ...
      target.takeDamage(existingMark.bonusDamage || this.damage, this.owner, 'MAGIC');
    }
