...
export default class Ekko_W extends Spell {
  targetingMode = 'POINT' as const;
  image = api.asset('spell_ekko_w');
  name = 'Lưỡng Giới Đồng Quy (Ekko_W)';
  description =
    `Bắn thiết bị tạo một vùng cầu thời gian tại điểm chỉ định sau <span class="time">2 giây</span>. Kẻ địch bên trong bị <span class="buff">Làm Chậm 40%</span>. Nếu Ekko đi vào vùng này, quả cầu phát nổ: cho Ekko Lớp Giáp ${heal(EKKO_W_SHIELD)} và <span class="buff">Làm Choáng</span> kẻ địch <span class="time">2.25 giây</span>.`;
  coolDown = 10000;
  manaCost = 50;
  range = 600;
...
export class Ekko_W_DeviceMissile extends MissileSpellObject {
  speed = 16;
  size = 20;
 ...
 */
export class Ekko_W_Object extends SpellObject {
...
export class Ekko_W_ShatterObject extends SpellObject {
  radius = EKKO_W_RADIUS;
  lifeTime = 450;
  timer = 0;
  shards: Shard[] = [];
