...
export default class Annie_R extends Spell {
  ...
  description = 
    ...
    ` vụ lửa xuất hiện gây ${dmg(SUMMON_DAMAGE, 'MAGIC')} trong <span>${SUMMON_RADIUS}px</span>.` +
    ` Tibbers có <span class="buff">${TIBBERS_HEALTH} máu</span>, tự đánh kẻ địch gần nhất và thiêu` +
    ` ${dmg(AURA_DAMAGE_PER_TICK, 'MAGIC')} mỗi nhịp quanh mình.` +
  ...
  onSpellCast() {
    ...
    enemies.forEach((enemy: any) => enemy.takeDamage(SUMMON_DAMAGE, this.owner, 'MAGIC'));
...
export class Tibbers extends Pet {
  ...
  update(): void {
    ...
    enemies.forEach((enemy: any) => {
      enemy.takeDamage(AURA_DAMAGE_PER_TICK, this.ownerUnit, 'MAGIC');
