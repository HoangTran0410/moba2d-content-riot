...
export default class MasterYi_E extends Spell {
  ...
  description =
    ...
    ` ${dmg(BONUS_TRUE_DAMAGE, 'TRUE')} và anh nhận` +
  ...
  onSpellCast(): void {
    ...
    this._stopWatching = this.game.eventManager.on(
      ...
      ({ attacker, victim }: BasicAttackHit) => {
        ...
        // `BONUS_TRUE_DAMAGE`, and the tooltip promises "sát thương chuẩn".
        victim.takeDamage(BONUS_TRUE_DAMAGE, this.owner, 'TRUE');
        blade.cut(victim);
      }
    );
  }
 ...
 */
export class MasterYi_E_Object extends SpellObject {
  age = 0;
  _cuts: WujuCut[] = [];
