...
export const isBounceTarget = (target: unknown): target is AttackableUnit =>
  target instanceof AttackableUnit && target.targetable && !target.toRemove && !target.isDead;


export class Brand_R_Fireball extends SpellObject {
  image = api.asset('spell_brand_r');
  ...
  _strike(victim: AttackableUnit) {
    ...
    victim.takeDamage(DAMAGE_PER_BOUNCE, this.owner, 'MAGIC');
