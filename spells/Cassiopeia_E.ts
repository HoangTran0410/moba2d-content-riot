 ...
 */
export const isPoisoned = (unit: AttackableUnit): boolean =>
 ...
 */
export class Cassiopeia_E_Venom extends MissileSpellObject {
  speed = SPIT_SPEED;
  size = 22;
  /**
  ...
  onArrive(): void {
    const target = this.target;
    if (target && !target.isDead && !target.toRemove) {
      target.takeDamage(this.poisoned ? POISONED_DAMAGE : BASE_DAMAGE, this.owner, 'MAGIC');
    }
