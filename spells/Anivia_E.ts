...
export const isFrostbiteTarget = (target: unknown): target is FrostbiteTarget =>
...
export default class Anivia_E extends Spell {
  ...
  description = `Anivia bắn một mũi băng vào mục tiêu, gây <span class="damage magic">${BASE_DAMAGE} sát thương phép</span>, tăng gấp đôi thành <span class="damage">${CHILLED_DAMAGE}</span> nếu mục tiêu đang <span class="buff">Nhiễm Lạnh</span> (bị Sương Băng hoặc Bão Tuyết đã hình thành đầy đủ đánh trúng gần đây).`;
...
export class Anivia_E_Bolt extends HomingMissileSpellObject {
  speed = MISSILE_SPEED;
  size = SIZE;
  baseDamage = BASE_DAMAGE;
  chilledDamage = CHILLED_DAMAGE;
...
export class Anivia_E_Impact extends SpellObject {
