...
describe('Leblanc R (Mimic)', () => {
  ...
  it('recasts the same ability at the same target, ignoring its cooldown and without paying its cost again', () => {
    ...
    //
    // A hand-rolled loop rather than `.filter((o): o is Leblanc_Q_Object =>
    ...
    // same fix elsewhere).
    const orbs: Leblanc_Q_Object[] = [];
    for (const object of owner.game.objectManager._objectToBeAdd) {
      if (object instanceof Leblanc_Q_Object) orbs.push(object);
    }
