...
describe('Jinx R drags a plume that is its own object', () => {
  ...
  it('balls the plume up where it detonates, under two separate pulses', () => {
    ...
    // gets a look in and the result comes back as `GameObject[]` (see
    // CLAUDE.md's own note on this trap, and `MatchDirector.bots()` for the
    // same fix elsewhere).
    const pulses: AoePulse[] = [];
    for (const object of game.objectManager._objectToBeAdd) {
      if (object instanceof AoePulse) pulses.push(object);
    }
    ...
    expect(new Set(pulses.map(pulse => pulse.style)).size).toBe(2);
  });
});
