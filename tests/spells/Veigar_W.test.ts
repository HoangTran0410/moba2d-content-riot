...
describe('Veigar W impact', () => {
  ...
  it('draws in both phases without blitting the ability icon, and covers the whole danger zone', () => {
    ...
    // `Veigar_W_Object extends SpellObject` directly, unlike a
    // `MissileSpellObject` (which declares `image?: AssetHandle` for its own
    // default-icon fallback) — this is proving that fallback structurally
    // cannot fire here, not reading a field the class actually declares.
    expect((strike as unknown as { image?: unknown }).image).toBeUndefined();
