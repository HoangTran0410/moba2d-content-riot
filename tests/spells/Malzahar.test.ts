...
import type { CastContext, GameObject } from '@moba2d/core/content/types';
import { installSketchMathGlobals, installSpellObjectGlobals } from '@moba2d/core/testing/spell';
import { DAMAGE as VOID_DAMAGE, DELAY_MS as VOID_DELAY_MS, PORTAL_GAP } from '../../spells/Malzahar_Q';
import makeMalzahar_Q, { makeMalzahar_Q_Object } from '../../spells/Malzahar_Q';
import { SPAWN_DELAY_MS, SPAWN_STAGGER_MS, VOIDLING_COUNT, VOIDLING_HEALTH } from '../../spells/Malzahar_W';
import makeMalzahar_W, { makeMalzahar_W_Rift, makeMalzahar_W_Voidling } from '../../spells/Malzahar_W';
import { MANA_ON_KILL, VISIONS_STACK_ID } from '../../spells/Malzahar_E';
import makeMalzahar_E, { makeMalzahar_E_Object } from '../../spells/Malzahar_E';
import { CHANNEL_DURATION_MS, ZONE_DAMAGE_PER_TICK, ZONE_TICK_MS } from '../../spells/Malzahar_R';
import makeMalzahar_R, { makeMalzahar_R_Zone } from '../../spells/Malzahar_R';
const __api = buildTestApi();
const { Champion } = __api.units;
type Champion = InstanceType<typeof __api.units.Champion>;
const { Silence, Stun } = __api.buffs;
...
// `T extends GameObject`, not a bare `T`: unconstrained, TS cannot prove the
// type-predicate overload of `Array.prototype.find` applies (`S extends T`
// where `T` here is `spawned()`'s own `GameObject`), and silently falls
// back to the plain boolean-returning overload — which returns `GameObject`,
// not the narrowed type, the moment this helper is called.
const find = <T extends GameObject>(is: (object: unknown) => object is T): T =>
  spawned().find(is)!;
...
describe('Malzahar W — Void Swarm', () => {
  it('hatches real bodies out of the rift, and none of them is a takedown', () => {
    ...
    // A hand-rolled loop rather than `.filter((o): o is Malzahar_W_Voidling
    // => ...)`: `src/types/global.d.ts` re-declares `Array.prototype.filter`
    // with the non-predicate overload first, so the type-predicate overload
    // never gets a look in and the result comes back as `GameObject[]` (see
    // CLAUDE.md's own note on this trap, and `MatchDirector.bots()` for the
    // same fix elsewhere).
    const swarm: Malzahar_W_Voidling[] = [];
    for (const object of spawned()) {
      if (object instanceof Malzahar_W_Voidling) swarm.push(object);
    }
