...
import { DAMAGE, EXPLODE_RADIUS, FULL_POWER_DISTANCE, MAX_SPEED, MAX_STUN_MS, MAX_TRAVEL, MIN_STUN_MS, SIZE, SPEED } from '../../spells/Ashe_R';
  ...
  return {
    ...
    game: {
      ...
      objectManager: {
        ...
        queryObjects: vi.fn(() => [] as unknown[]) as unknown as (...args: unknown[]) => unknown[],
      },
    },
    addBuff: vi.fn(),
    objects,
...
describe('Ashe R', () => {
  ...
  it('stuns and damages on the first enemy it touches, using the stun icon rather than its own ability art', () => {
    ...
    expect(enemyBuffs[0].image).toBe(__api.asset('buff_stun'));
  });
