...
describe('Janna W', () => {
  ...
  it('grants a permanent ghost and speed passive that survives repeated updates without stacking', () => {
    ...
    // A hand-rolled loop rather than `owner.buffs.filter((b): b is
    // Janna_W_Passive => ...)`: `src/types/global.d.ts` re-declares
    // `Array.prototype.filter` with the non-predicate overload first, so the
    // type-predicate overload never gets a look in and the result comes
    // back typed as `Buff[]` (see CLAUDE.md's own note on this trap, and
    // `MatchDirector.bots()` for the same fix elsewhere).
    const passives: Janna_W_Passive[] = [];
