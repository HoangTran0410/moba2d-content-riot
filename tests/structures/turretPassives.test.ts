...
import makeTurretPassives, {
  ...
  WARMING_UP_RESET_MS,
} from '../../structures/Turret';

/**
 * **What this pack's towers are built carrying.**
 *
 * Three named passives from the source game, written as real buffs on the
 * turret rather than branches inside core's `Turret` — which is the whole point
 * of the `turretPassives` slot. What is asserted here is that each of them is
 * plugged into a real engine seam, because that is exactly the claim that goes
 * on reading true in a file's prose long after it has stopped being true in its
 * code: a ramp that raised a field nothing reads still looks like a ramp.
 */
...
const turret = (teamId = TeamId.BLUE, x = 0) =>
  new Turret({
    ...
    teamId,
    preset: { ...DEFAULT_TURRET_PRESET, passives: makeTurretPassives(api) },
  } as never);
...
beforeEach(() => {
  ...
  game.setPlayer(new api.units.Champion({ game, teamId: 'player-uuid' } as never) as never);
});
...
describe('Ohmwrecker', () => {
  ...
  it('gets angrier with every champion it hits, up to the ceiling', () => {
    ...
    expect(tower.damage).toBeCloseTo(
      base * (1 + WARMING_UP_PER_STACK * WARMING_UP_MAX_STACKS),
      4
    );
  });
...
describe("Warden's Eye", () => {
  ...
  it('leaves an ally alone', () => {
    ...
    friend.addBuff(new api.buffs.Invisible(5_000, friend, friend));
