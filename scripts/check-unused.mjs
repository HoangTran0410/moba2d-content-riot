 ...
 *
 * Why it exists at all: nothing ever asked, and 1968 dead declarations
 * accumulated in `spells/` — 1697 type aliases and 271 `const X = api.Y`
 * value aliases, emitted per file by the codemod that first moved this pack
 * onto the injected `api`, none of them read by anything. That is not a
 * formatting complaint; it is what made these files read as machine output
 * and buried the ~40 lines per file that are actually the ability.
 *
 * The last twelve were not aliases and are worth knowing about, because they
 * are the shape this catches from now on: six imports of constants a test had
 * stopped using, two `{ from, to }` destructures reading a half nobody wanted,
 * a private field nothing touched, and two values computed inside a `draw()`
 * and then dropped — `Graves_W`'s crown lobes still compute the `wobble` the
 * pile lobes below them apply, and never apply it.
 */
